// from https://github.com/google/diff-match-patch/blob/master/javascript/diff_match_patch_uncompressed.js
...
export class diff_match_patch {
    ...
    private diff_linesToChars_(text1: string, text2: string): {
        ...
        var lineHash: { [key in string]: number } = {};   // e.g. lineHash['Hello\n'] == 4
     ...
     * @param {!Array.<!Diff>} diffs Array of diff tuples.
     */
    public diff_cleanupSemantic(diffs: Diff[]) {
        var changes = false;
     ...
     * @private
     */
    private diff_cleanupSemanticScore_(one: string, two: string) {
        if (!one || !two) {
            // Edges are the best.
            return 6;
        }
     ...
     * @param {!Array.<!Diff>} diffs Array of diff tuples.
     */
    public diff_cleanupSemanticLossless(diffs: Diff[]) {
        var pointer = 1;
     ...
     * @param {!Array.<!Diff>} diffs Array of diff tuples.
     */
    public diff_cleanupEfficiency(diffs: Diff[]) {
        var changes = false;
        var equalities = [];  // Stack of indices where equalities are found.
        var equalitiesLength = 0;  // Keeping our own length var is faster in JS.
        /** @type {?string} */
     ...
     * @param {!Array.<!Diff>} diffs Array of diff tuples.
     */
    public diff_cleanupMerge(diffs: Diff[]) {
        // Add a dummy entry at the end.
        diffs.push(new Diff(DIFF_EQUAL, ''));
        var pointer = 0;
        var count_delete = 0;
     ...
     */
    public diff_prettyHtml(diffs: Diff[]) {
        var html = [];
        var pattern_amp = /&/g;
        var pattern_lt = /</g;
        var pattern_gt = />/g;
     ...
     */
    public diff_levenshtein(diffs: Diff[]) {
        var levenshtein = 0;
        var insertions = 0;
        var deletions = 0;
        for (var x = 0; x < diffs.length; x++) {
     ...
     */
    public diff_toDelta(diffs: Diff[]) {
        var text = [];
        for (var x = 0; x < diffs.length; x++) {
            switch (diffs[x][0]) {
                case DIFF_INSERT:
     ...
     * @private
     */
    private match_alphabet_(pattern: string): { [key in string]: number } {
        var s: { [key in string]: number } = {};
     ...
     * @return {!Array.<!patch_obj>} Array of Patch objects.
     */
    public patch_deepCopy(patches: patch_obj[]): patch_obj[] {
     ...
     */
    public patch_toText(patches: patch_obj[]) {
        var text = [];
        for (var x = 0; x < patches.length; x++) {
            text[x] = patches[x];
        }
     ...
     * @throws {!Error} If invalid input.
     */
    public patch_fromText(textline: string): patch_obj[] {
...
export class Diff {
     ...
     * @constructor
     */
    constructor(op: number | typeof DIFF_DELETE | typeof DIFF_INSERT | typeof DIFF_EQUAL, text: string) {
        this[0] = op;
        this[1] = text;
    }
